...
def do_train(
    ...
    for iteration, ((images_s, targets_s, _), (images_t, _, _)) \
        ...
        for layer in used_feature_layers:
            ...
            if USE_KTNET:
                IK_loss, source_feature["source_feature_%s" % layer], = \
                    model["dis_%s_KTNet" % layer](features_s[layer], source_label, score_maps_s[layer], domain='source')
                loss_dict["loss_%s_IK_ds" % layer] = ca_dis_lambda * IK_loss 
        losses = sum(loss for loss in loss_dict.values())
        ...
        for layer in used_feature_layers:
            ...
            if USE_KTNET:
                IK_loss, RK_loss = \
                    model["dis_%s_KTNet" % layer](features_t[layer], target_label, score_maps_t[layer], domain='target', \
                        source_feature=source_feature["source_feature_%s" % layer].detach())
                loss_dict["loss_%s_IK_dt" %layer] = ca_dis_lambda * IK_loss 
                loss_dict["loss_%s_RK_dt" %layer] = RK_loss
        losses = sum(loss for loss in loss_dict.values())
        ...
        del loss_dict, losses, source_feature,
